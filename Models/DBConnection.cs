...
namespace NextHolidaysIn.Models
{
    ...
    {
        string connectionStr = "Data Source=74.225.201.149;Initial Catalog=TravelProject;User ID=BlueberryuatDB;Password=Blueberryuatadmin#4321;";
        public async Task<DataSet> GetDataSetAsync(String SPName, SqlParameter[] para, CancellationToken cts, int timeout)
        {
        ...
        public async Task<DataTable> GetDataTableAsync(String SPName, SqlParameter[] para, CancellationToken cts, int timeout)
        {
            return await Task.Run(() =>
            {
                using (SqlConnection con = new SqlConnection(connectionStr))
